...
def getFullPath(path):
    return os.environ['CMSSW_BASE']+"/src/OutreachExercise2010/FourLeptons/"+path
...
class Analyzer (object):
    ...
    def muonID(self,muon,vertex): #defined in FourLeptons
        return True

    def electronID(self,electron,vertex): #defined in FourLeptons
        return True
    ...
    def analyze(self,box): #defined in FourLeptons
        return True

    def declareHistos(self): #defined in FourLeptons
        return True

    def fillHistos(self,box,sample,weight = 1): #defined in FourLeptons
        return True

    def declareHisto(self,name,bins,min,max,xlabel=''): #used in FourLeptons
        for sample in self.samples:
    ...
    def fillHisto(self,name,sample,value,weight = 1): #used in FourLeptons
    ...
    def makePlot(self,histogram):
        ...
        frame = canvas.DrawFrame(self.histograms['data'][histogram].GetXaxis().GetXmin(),0.0,self.histograms['data'][histogram].GetXaxis().GetXmax(),10)
	...
	text = pt.AddText(0.39,0.4,"                             #sqrt{s} = 7 TeV, L = xy pb^{-1}")
	pt.Draw()   

        plot={'canvas':canvas,'legend':legend,'dataG':dataG,'latex1':pt}
